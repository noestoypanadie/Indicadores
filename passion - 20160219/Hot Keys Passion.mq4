...
//   }
   if (id == CHARTEVENT_KEYDOWN) {
      if(lparam == 90){ //letra z
         disminuyeTimeFrame();
      } else if(lparam == 88){ // letra x
         aumentaTimeFrame();
      } else if(lparam == 49){ // numero 1
        ChartApplyTemplate(0,"Limpia.tpl");
      } else if(lparam == 50){ // numero 2
         ChartApplyTemplate(0,"tripleMedia.tpl");
      } else if(lparam == 65){ // letra a
         limpiaIndicadores();
      } else if(lparam == 66){ // letra b
         hectorSetupChart();
      } else if(lparam == 67){ // letra c
         cambiarColorFondo();
      }
   }
}
...
void limpiaIndicadores(){ //TODO Queda definir qué indicadores se van a quedar siempre. P.ej. el MyHotKeys
   ...
   for(int v=0; v<=numeroWindows; v++){
      ...
      for(int  i=0; i<numeroIndicadores ; i++){
         ...
         if(indicatorName!="MyHotKeys" && indicatorName!="FINAL3.CustomIndicators4.3SMATrendFilterPro"){
