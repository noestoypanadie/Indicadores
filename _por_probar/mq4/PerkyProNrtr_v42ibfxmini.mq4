...
double MaxLots = 100;
extern bool Wait_For_Fresh_Cross=false;      //waits for fresh cross on indicator to open trade instead of instant open
 
//+---------------------------------------------------+
//|Profit controls                                    |
//+---------------------------------------------------+
extern double StopLoss = 40;        // Maximum pips willing to lose per position.
...
extern double TrailingStop = 25;    // Change to whatever number of pips you wish to trail your position with.
 double Margincutoff = 100;   // Expert will stop trading if equity level decreases to that level.
...
int init() 
{
   ...
   setup="ProNrtr_v3 " + Symbol() + "_" + func_TimeFrame_Val2String(func_TimeFrame_Const2Val(Period()));
...
bool CheckCross()
{
   double proup,NRTRup,proupb4,NRTRupb4;
   double prodown,NRTRdown,prodownb4,NRTRdownb4;
     ...
     if (UseNRTR == 1)
     {
        ...
        NRTRupb4  =iCustom(Symbol(),Period(),"NRTR_color_line",0,SignalCandle+1); //up
        NRTRdownb4=iCustom(Symbol(),Period(),"NRTR_color_line",1,SignalCandle+1);//down
     }
     else
     { 
       NRTRup     =1; //force this lot if not using NRTR
       NRTRupb4   =1;
       NRTRdown   =1;
       NRTRdownb4 =1;
     } 
     if (proup<9999   && NRTRup>0   && proupb4<9999   && NRTRupb4>0)   return(false);
     if (prodown<9999 && NRTRdown>0 && prodownb4<9999 && NRTRdownb4>0) return(false);
     checked=true;   //only check once on opening of expert - saves running all this again (maybe) not installed yet
     return(true);
...
// Was there a sudden jump?  Ignore it...
 ...
 // }
   ...
   //+------------------------------------------------------------------+
   //| Check for new cross0Check for Open Position                                          |
   //+------------------------------------------------------------------+
  if (Wait_For_Fresh_Cross && !checked && !CheckCross() ) return(0);
...
double LotsOptimized()
  {
   double lot=Lots;
//---- select lot size
   lot=NormalizeDouble(MathFloor(AccountFreeMargin()*TradeSizePercent/100)/10,.01);
