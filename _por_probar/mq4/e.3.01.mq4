...
extern bool       BreakEvenSL       =true;   //enables/disables break even SL
extern int        BreakEvenPips     =2;      //pips to break even/cover swap/prevent loss

int               b1,b2,b3,s1,s2,s3;
double            TSL               =0;
string            comment           ="m e.3.02 ";
...
int start() {
   ...
   for(cnt=0;cnt<total;cnt++) {
      OrderSelect(cnt, SELECT_BY_POS, MODE_TRADES);            
      if(OrderType()==OP_BUY && IsTradeAllowed()) {
         ...
         if(Bid>OrderOpenPrice())   {
            if((/*Close[0]>bline) && (*/TSL>OrderStopLoss()))  {
               ...
               OrderModify(OrderTicket(),
                           ...
                           bsl,
                           OrderTakeProfit(),
                           OrderExpiration(),//Order expiration server date/time
                           Green);  }  }  }
      if(OrderType()==OP_SELL && IsTradeAllowed())   {
         if(MaElineTSL==0) {TSL=NormalizeDouble(ma,Digits); }
         ...
         if(Ask<OrderOpenPrice())   {
            if((/*Close[0]<sline) && (*/TSL<OrderStopLoss()))  {
               ...
               OrderModify(OrderTicket(),
                           ...
                           OrderExpiration(),//Order expiration server date/time
                           Red); }  }  }      
/*      if(Hour()==TimeDelete && OrderType()==OP_BUYSTOP && IsTradeAllowed())  {
         OrderDelete(OrderTicket());
         if(OrderTicket()==b1) {b1=0; return;}
         if(OrderTicket()==b2) {b2=0; return;}
         if(OrderTicket()==b3) {b3=0; return;}  }
      if(Hour()==TimeDelete && OrderType()==OP_SELLSTOP && IsTradeAllowed()) {
         OrderDelete(OrderTicket());
...
//if(IsTesting())   Print(TimeDelete,"  ",TimeToStr(TimeDelete,TIME_MINUTES));
return(0);   
}
