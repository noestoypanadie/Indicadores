...
extern bool    Exit.On.Yellow=false;                              // exit a position if yellow appears on t/f specified below and all t/f above that
...
extern bool    Use.Stoch.For.Entry=true;                          // if true, entries will be filtered by stoch (15/5/5) above or below 50 level
extern string  SL_TP_Trail_Options="--------------------------";
extern int     StopLoss.Pips=100;                                 // static, initial s/l. Unused if Use.Adr.for.sl.pips = true
extern int     TakeProfit.Pips=300;                               // static, initial take profit
extern int     Trail.Pips=10;                                     // trail.pips. Unused if Use.Adr.for.sl.pips=true or if value=0
...
extern int     Move.To.BreakEven.at.pips=20;                      // trades in profit will move to entry price + Move.To.BreakEven.Lock.pips
                                                                  // as soon as trade is at entry price + Move.To.BreakEven.at.pips
extern int     Move.To.BreakEven.Lock.pips=1;
extern int     Move.Trail.Every.xx.Pips=30;                       // If > 0 then ALL other s/l are dropped and trail will only move by
...
string DR, DR1, comment=" TST v1.20",ScreenComment="TradeStarTrend[EA] v1.20";
...
double ADXMain1, PlusDI1, MinusDI1, ADXMain2, PlusDI2, MinusDI2, CurrentJuice, TPPrice, StochValue, StochValue0;
...
{  if (Use.Adx)
   ...
   if (Use.Stoch.For.Entry)
   {  StochValue  = iStochastic(NULL,Trigger.Period,15,5,5,MODE_SMA,0,MODE_MAIN,1);
      StochValue0 = iStochastic(NULL,Trigger.Period,15,5,5,MODE_SMA,0,MODE_MAIN,0);
   }
}
...
{  bool StarTrendIsOK;
   ...
   if (!Use.Stoch.For.Entry || (Use.Stoch.For.Entry && StochValue>=50 && StochValue0>=50)) {StochOK=true;}
   
   if ((StarTrendIsOK) && (StochOK)) return(1);
   ...
   if (!Use.Stoch.For.Entry || (Use.Stoch.For.Entry && StochValue<=50 && StochValue0<=50)) {StochOK=true;}
   if ((StarTrendIsOK) && (StochOK)) return(2);
   return(0);
}
