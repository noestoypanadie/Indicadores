...
extern int       blTakePartialProfitAtBEtime=0; 
...
int start()
   ...
   */  
   ...
   if(blCancelPendingAtStartTime == 1 && CurTime()> StartTime-15*60 && CurrH1BarTime<StartTime){
         for (i=0;i<OrdersTotal();i++){
         OrderSelect(i,SELECT_BY_POS,MODE_TRADES);
         if(OrderSymbol()==Symbol() && (OrderType()==OP_BUYSTOP || OrderType()==OP_SELLSTOP)) OrderDelete(OrderTicket());
         }
       }
