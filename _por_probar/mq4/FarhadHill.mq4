...
extern double ThirdMove = 50;       // type 3 third level pip gain
...
// Check MACD Rules
   ...
   macdHistCurrent     = iMACD(NULL,0,12,26,9,MACD_Price,MODE_MAIN,SignalCandle);   
//   macdHistPrevious    = iMACD(NULL,0,12,26,9,MACD_Price,MODE_MAIN,SignalCandle+1);   
   macdSignalCurrent   = iMACD(NULL,0,12,26,9,MACD_Price,MODE_SIGNAL,SignalCandle); 
//   macdSignalPrevious  = iMACD(NULL,0,12,26,9,MACD_Price,MODE_SIGNAL,SignalCandle+1);
    
   if (macdHistCurrent>=macdSignalCurrent) return (false);

// Check Stochastic rules

   stochHistCurrent    = iStochastic(NULL,0,5,3,3,Stoch_Mode,StochPrice,MODE_MAIN,SignalCandle);
//   stochHistPrevious   = iStochastic(NULL,0,5,3,3,Stoch_Mode,StochPrice,MODE_MAIN,SignalCandle+1);
//   stochSignalCurrent  = iStochastic(NULL,0,5,3,3,Stoch_Mode,StochPrice,MODE_SIGNAL,SignalCandle);
//   stochSignalPrevious = iStochastic(NULL,0,5,3,3,Stoch_Mode,StochPrice,MODE_SIGNAL,SignalCandle+1);
   
   if (stochHistCurrent>=StochLow) return(false);
   
// Check Paraboloc SAR rules

   sarCurrent          = iSAR(NULL,0,0.02,0.2,SignalCandle);           // Parabolic Sar Current
   if (sarCurrent>Ask) return (false);
   
   sarPrevious         = iSAR(NULL,0,0.02,0.2,SignalCandle+1);         //Parabolic Sar Previous
   if (sarPrevious<=sarCurrent) return (false);
   
// Check Momentum rules

   momCurrent          = iMomentum(NULL,0,MomentumPeriod,PRICE_OPEN,SignalCandle); // Momentum Current
//   momPrevious         = iMomentum(NULL,0,MomentumPeriod,PRICE_OPEN,SignalCandle+1); // Momentum Previous
   if(momCurrent>=MomentumLow) return (false);
   
//   highCurrent         = iHigh(NULL,0,SignalCandle);     //High price Current
//   lowCurrent          = iLow(NULL,0,SignalCandle);      //Low Price Current

/*
   if (MA_Mode == 4)
   {
     maLongCurrent = LSMA(MA_SlowPeriod,MA_Price,0,SignalCandle);
     maLongPrevious = LSMA(MA_SlowPeriod,MA_Price,0,SignalCandle+1);
     maShortCurrent = LSMA(MA_FastPeriod,MA_Price,0,SignalCandle);
     maShortPrevious = LSMA(MA_FastPeriod,MA_Price,0,SignalCandle+1);
   }
   else
   {
     maLongCurrent       = iMA(NULL,0,MA_SlowPeriod,MA_Shift,MA_Mode,MA_Price,SignalCandle); 
     maLongPrevious      = iMA(NULL,0,MA_SlowPeriod,MA_Shift,MA_Mode,MA_Price,SignalCandle+1); 
     maShortCurrent      = iMA(NULL,0,MA_FastPeriod,MA_Shift,MA_Mode,MA_Price,SignalCandle); 
     maShortPrevious     = iMA(NULL,0,MA_FastPeriod,MA_Shift,MA_Mode,MA_Price,SignalCandle+1); 
   }
*/
