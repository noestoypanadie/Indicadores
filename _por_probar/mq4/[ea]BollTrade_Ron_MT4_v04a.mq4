...
//v4a - added RefreshRates to every error condition open AND close 
//      added tickwrite to track close[0] when outside the bands. Some trades are being
//      missed and I'm not sure why. NormalizeDouble is likely next everywhere
...
string TradeComment = "_bolltrade_v03f.txt";
...
int start()
  {
   ...
   if(Close[0]>bup) tickwrite("BollTick.txt","Bdistance="+BDistance+" BUP="+bup+" Close[0]="+Close[0]+" Diff="+udiff  );
   if(Close[0]<bdn) tickwrite("BollTick.txt","Bdistance="+BDistance+" BDN="+bdn+" Close[0]="+Close[0]+" Diff="+ddiff  );
...
void logwrite (string filename, string mydata)
  {
   ...
   boo=TimeToStr(CurTime(),TIME_DATE|TIME_SECONDS);
   myhandle=FileOpen(Symbol()+"_"+filename, FILE_CSV|FILE_WRITE|FILE_READ, ";");
   if(myhandle>0)
     {
      FileSeek(myhandle,0,SEEK_END);
      FileWrite(myhandle, mydata+" "+CurTime()+" "+boo);
      FileClose(myhandle);
     }
  } 

void tickwrite (string filename, string mydata)
  {
   int myhandle;
   string boo;
   
   //TimeToStr(CurTime()-TimeZoneDiff*60*60,TIME_DATE|TIME_SECONDS)
   //if you want it to secs, work?
   //TIME_MINUTES for mins. Omit for date only.
