...
extern string MinutesBeforeNews_is=            "Minutes BEFORE news event to avoid trades ";
extern int    MinutesBeforeNews   =      16;   
extern string MinutesAfterNews_is =            "Minutes AFTER news event to avoid trades  ";
extern int    MinutesAfterNews    =      16;
...
string TradeComment = "_bolltrade_v03mD3.txt";
...
int start()
  {
   ...
   // set false 1st, then we can check multiple related pairs   
   NewsTime = false;
   
   if( IsNewsTime(MinutesBeforeNews, MinutesAfterNews, Symbol()) ) 
     {
      NewsTime = true;
      Comment("TRUE   IsNewsTime   TRUE\n");
     }
    else
     {
      Comment("FALSE   IsNewsTime   FALSE\n");
     }

   // if close is above upper band + BDistance then SELL
   if(Close[0]>bux && volumeOK && TradeDay && !NewsTime) 
     {
      SELLme=true; 
   ...
   if(Close[0]<bdx && volumeOK && TradeDay && !NewsTime) 
     {
      BUYme=true;  
