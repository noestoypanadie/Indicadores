//+------------------------------------------------------------------+
//|                                   TSD_MR_Trade_MACD_WPR_0_24.mq4 |
...
bool TrailStop (int i, int TrailingStop) {
   ...
   if ( OrderType() == OP_BUY ) {
      if ( MarketInfo (TradeSymbol, MODE_BID) < OrderOpenPrice () )  return(false);
   ...
   if ( OrderType() == OP_SELL ) {
      if ( MarketInfo (TradeSymbol, MODE_ASK) > OrderOpenPrice () )  return(false);
