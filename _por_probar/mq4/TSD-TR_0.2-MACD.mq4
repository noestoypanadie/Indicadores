...
if (newbar != TimeDayOfWeek(CurTime())) 
{
	 ...
	 //Newbar will be reset to 999 if the once-a-day order operations below fail, to ensure that they are retried on the next tick.
	  ...
	  if (newbar<7) //Modify this to switch to using current week's OSMA on a specific day of the week (0-Sunday,1,2,3,4,5,6) 
         {
         OsMAPrevious  = iMACD(NULL,10080,12,26,9,PRICE_CLOSE,MODE_MAIN,1);
	      OsMAPrevious2 = iMACD(NULL,10080,12,26,9,PRICE_CLOSE,MODE_MAIN,2);
         }
         else
         {
         OsMAPrevious  = iMACD(NULL,10080,12,26,9,PRICE_CLOSE,MODE_MAIN,0);
	      OsMAPrevious2 = iMACD(NULL,10080,12,26,9,PRICE_CLOSE,MODE_MAIN,1);
         }
