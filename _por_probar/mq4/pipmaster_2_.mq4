...
if( mm != 0 ) 
lotsi=MathCeil(AccountBalance()*risk/1000)/10;
 else 
 lotsi=Lots;
...
if( orders>0 ) {
 if( OrderValueType(sym) == OP_SELL ) {
     OrderClose(OrderValueTicket(sym),OrderValueLots(sym),Bid,slippage,Red);
     return(0); 
 }
 if( OrderValueType(sym) == OP_BUY ) {
     OrderClose(OrderValueTicket(sym),OrderValueLots(sym),Ask,slippage,Cyan); 
     return(0); 
 }
}
if( ptime != Time[0] && buy ) {
   if( orders == 0 ) {
  ptime=Time[0];
        MOrderSend(Symbol(),OP_BUY,lotsi,Ask,slippage,Bid-StopLoss*Point,Ask+TakeProfit*Point,"",16384,0,Cyan);
        return(0); 
   }
}
if( ptime != Time[0] && sell ) {
   if( orders == 0 ) {
  ptime=Time[0];
        MOrderSend(Symbol(),OP_SELL,lotsi,Bid,slippage,Ask+StopLoss*Point,Bid-TakeProfit*Point,"",16384,0,Red);
        return(0);
    } 
}
  return(0);
