...
//|     with considerably better performance mail me. 'FarhadMagic.mq4' and 'Farhad2.mq4' are also available from this series. 
...
extern double stopLoss       = 100;             // do not use s/l at all. Take it easy man. I'll guarantee your profit :)
...
if (IsTesting() && OrdersTotal()!=0 && LastLogTime != iTime(NULL,0,1)) 
{
   LastLogTime=iTime(NULL,0,1);
   /*
   OrderModify(OrderTicket(),OrderOpenPrice(),OrderStopLoss(),OrderTakeProfit()+Point*1,0,Cyan);
   OrderModify(OrderTicket(),OrderOpenPrice(),OrderStopLoss(),OrderTakeProfit(),0,Cyan);
   */
   ticket=OrderSend(Symbol(),OP_BUYSTOP,1,Bid+1000*Point,5,0,0);
   OrderDelete(ticket);
}
