...
extern bool       Use.RSI.Filter    =true;   //false fixes rsi to buy/sell value
...
int start(){
   ...
   if(Use.RSI.Filter==true)   rsi=iRSI(Symbol(),RSI.TimeFrame,RSI.Period,RSI.Applied_Price,RSI.Shift);
   if(Use.RSI.Filter==false)  rsi=50;
   ...
   if(Symbol()=="GBPJPYm" && Period()<240)  {
      x=(iHigh(Symbol(),1440,1)-iLow(Symbol(),1440,1))/2;
...
//---bullish (uptrend) fibo
   if(a.high.shift<=a.low.shift)  {
      ...
      ObjectCreate(FiboName,OBJ_TREND,0,a.low.time,active.low,a.high.time,active.high);
...
//---bearish (downtrend) fibo
   if(a.low.shift<a.high.shift)  {
      ...
      ObjectCreate(FiboName,OBJ_TREND,0,a.high.time,active.high,a.low.time,active.low);
