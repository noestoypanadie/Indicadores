...
extern int ManualBuy  = 0; // default = 0 any other value and the expert will set the selected type trade and manage it .
extern int ManualSell = 0; // default = 0 any other value and the expert will set the selected type trade and manage it .
 ...
 { // A
  ...
  if (UserAcceptsAllLiability == true)
   { // B
    ...
    if ( (ManualBuy == 0) && (ManualSell == 0) ) ManualSig = 0;
   ...
   // Count Open Trades and record OrderOpenTime --------------------------
     ...
     { // 1
      ...
      if ( (OrderSymbol() == Symbol()) && (OrderMagicNumber() == magic) && (OrderOpenTime() > 0) ) // check symbol , magic num and find latest trade time .
       { // 2
        ...
        if (OrderTime >= Time[0]) openperiod = -10;
        if (OrderTime < Time[0]) openperiod =  10;
       } // 2
     } // 1
    ...
    // Calculate Indicators -------------------------------------------------
     ...
     ofast1 = iMA(Symbol(), 0, mafastperiod, mafastshift, mafastmethod, mafastprice, 0 + EntryBar)*1000;
     ofast2 = iMA(Symbol(), 0, mafastperiod, mafastshift, mafastmethod, mafastprice, 1 + EntryBar)*1000;
     oslow1 = iMA(Symbol(), 0, maslowperiod, maslowshift, maslowmethod, maslowprice, 0 + EntryBar)*1000;
     oslow2 = iMA(Symbol(), 0, maslowperiod, maslowshift, maslowmethod, maslowprice, 1 + EntryBar)*1000;
     
     cfast1 = iMA(Symbol(), 0, mafastperiod, mafastshift, mafastmethod, mafastprice, 0 + CloseBar)*1000;
     cfast2 = iMA(Symbol(), 0, mafastperiod, mafastshift, mafastmethod, mafastprice, 1 + CloseBar)*1000;
     cslow1 = iMA(Symbol(), 0, maslowperiod, maslowshift, maslowmethod, maslowprice, 0 + CloseBar)*1000;
     cslow2 = iMA(Symbol(), 0, maslowperiod, maslowshift, maslowmethod, maslowprice, 1 + CloseBar)*1000;
