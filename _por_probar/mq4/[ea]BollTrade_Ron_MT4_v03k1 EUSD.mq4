...
extern bool   KillLogging  =   false ;   // don't do ANY logging
...
string TradeComment = "_bolltrade_v03k1.txt";
...
int start()
  {
   ...
   string myCmt;
   if(Close[0]>bup)                 myCmt="S Up="+bup+" Dn="+bdn+" Close="+Close[0]+" Vol_1="+Volume[1]+" Vol_0="+Volume[0];
   if(Close[0]<bdn)                 myCmt="B Up="+bup+" Dn="+bdn+" Close="+Close[0]+" Vol_1="+Volume[1]+" Vol_0="+Volume[0];
   if(Close[0]<bup && Close[0]>bdn) myCmt="X Up="+bup+" Dn="+bdn+" Close="+Close[0]+" Vol_1="+Volume[1]+" Vol_0="+Volume[0];
   Comment(myCmt);
   logwrite("bup_bdn.txt",myCmt); 
   ...
   if(Close[0]>bup+(BDistance*Point) && volumeOK && TradeDay) 
     {
      ...
      if(logging) logwrite("bup_bdn.txt","---SELLme happened"); 
      if(Compounding && CompoundTrack>0 && Close[0]>CompoundTrack)
        {
         ...
         if(logging) logwrite("bup_bdn.txt","---SELLme happened again "+CompoundTrack); 
        }
     }
   ...
   if(Close[0]<bdn-(BDistance*Point) && volumeOK && TradeDay) 
     {
      ...
      if(logging) logwrite("bup_bdn.txt","---BUYme happened"); 
      if(Compounding && CompoundTrack>0 && Close[0]<CompoundTrack)
        {
         BUYme=true;
         TradeAllowed=true;
         if(logging) logwrite("BollTick3g.txt","----BUYme happened again "+CompoundTrack ); 
         if(logging) logwrite("bup_bdn.txt","---BUYme happened again "+CompoundTrack); 
        }
     }
         ...
         if(gle==0)
           {
            ...
            //CompoundTrack=Ask;
            if(logging) logwrite("bup_bdn.txt","BUY Ticket="+ticket+" bdn="+bdn+" Ask="+Ask+" Lots="+Lots+" SL="+SL+" TP="+TP); 
         ...
         if(gle==0)
           {
            CompoundTrack=Close[0]-(CompoundStep*Point);
            //CompoundTrack=Bid;
            if(logging) logwrite("bup_bdn.txt","SELL Ticket="+ticket+" bup="+bup+" Bid="+Bid+" Lots="+Lots+" SL="+SL+" TP="+TP);
            if(logging) logwrite(TradeComment,"                                                                 ");
