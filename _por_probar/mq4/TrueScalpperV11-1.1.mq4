...
double CalcTpBuy()   { return (MathMax((High[Highest(NULL,0,MODE_HIGH,13,0)]),(Ask+(TakeProfit*Point)))); }
double CalcTpSell()  { return (MathMax((Low[Lowest(NULL,0,MODE_LOW,13,0)]),(Bid-(TakeProfit*Point)))); }
   ...
   if(OrdersPerSymbol==0)
     {
      ...
      if(bullMA3>(bearMA7+p) && RSINEG)
         ...
         OrderSend(Symbol(),OP_BUY,LotsOptimized(),Ask,Slippage,SL,TP,"TS v11-1.1",0,0,White);
         bartick=0;
        }
      ...
      if(bullMA3<(bearMA7-p) && RSIPOS)
        {
         ...
         OrderSend(Symbol(),OP_SELL,LotsOptimized(),Bid,Slippage,SL,TP,"TS v11-1.1",0,0,Red);
         bartick=0;
        }
     } //if
