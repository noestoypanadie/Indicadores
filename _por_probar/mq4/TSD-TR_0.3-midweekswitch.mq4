...
double MacdCurrent=0, MacdPrevious=0, MacdPrevious2=0, Direction=0, OsMAPrevious=0, OsMAPrevious2=0, WeeklyDirection=0;
...
Comment("TSD for MT4 ver beta 0.3 - DO NOT USE WITH REAL MONEY YET",
        ...
        "\n","Weekly Direction = ",Direction,"    Weekly WeeklyDirection = ",WeeklyDirection,
        "\n",
...
if (newbar != TimeDayOfWeek(CurTime())) 
{
      ...
      */
	  ...
	  if (MacdPrevious > MacdPrevious2) Direction = Up;
	  if (MacdPrevious < MacdPrevious2) Direction = Down;
	  //if (MacdPrevious == MacdPrevious2) Direction = 0; 
	  
	  if (OsMAPrevious > OsMAPrevious2) WeeklyDirection = Up;
	  if (OsMAPrevious < OsMAPrevious2) WeeklyDirection = Down;
	  //if (OsMAPrevious == OsMAPrevious2) WeeklyDirection = 0;
...
/////////////////////////////////////////////////
   ...
   total=OrdersTotal();
	 ...
	 if(TradesThisSymbol < 1) 
	 {
	   ...
	   if(WeeklyDirection == 1 && WilliamsBuy)
		{
     ...
     if(WeeklyDirection == -1 && WilliamsSell)
     {
   ...
   if(TradesThisSymbol > 0)
     {
      for (cnt=0;cnt<OrdersTotal();cnt++) 
         ...
         if ( OrderSymbol()==Symbol() && (OrderType()==OP_BUY || OrderType()==OP_SELL))
            {
            int TSmultiplier=5;
            double b=5*Point+iATR(Symbol(),PERIOD_M15,3,1)*TSmultiplier;
            }
      ...
      if (OrderType()==OP_BUY)   
       ...
       if (Close[1]-OrderOpenPrice()>b)
        ...
        if (OrderStopLoss()<Close[1]-b) 
         {
         OrderModify(OrderTicket(),OrderOpenPrice(),Close[1]-b,Ask + TakeProfit * Point,0,LimeGreen);
         return(0);
         } 
        }
       }  
       ...
       if (OrderType()==OP_SELL) 
         ...
         if (OrderOpenPrice()-Close[1]>(b)+MarketInfo(Symbol(),MODE_SPREAD)*Point) 
            ...
            if (OrderStopLoss()>(Close[1]+b+MarketInfo(Symbol(),MODE_SPREAD)*Point)) 
              {
              OrderModify(OrderTicket(),OrderOpenPrice(),Close[1]+b+MarketInfo(Symbol(),MODE_SPREAD)*Point,Bid - TakeProfit * Point,0,HotPink);
              return(0);
              }
            }
         }
