...
double CalcTpBuy()   { return (MathMin((High[Highest(NULL,0,MODE_HIGH,BarsTP,BarShift)]-Spread),(Ask+(TakeProfit*Point)))); }
double CalcTpSell()  { return (MathMax((Low[Lowest(NULL,0,MODE_LOW,BarsTP,BarShift)]+Spread),(Bid-(TakeProfit*Point)))); }
...
bartick++;       }
   ...
   for(i = 0; i < OrdersTotal(); i++ ) {
      ...
      if(OrderMagicNumber()==0 &&
         OrderSymbol()==Symbol())  {
         btsl=(Low[Lowest(NULL,0,MODE_LOW,BarsTSL,BarShift)]-Spread);               
         stsl=(High[Highest(NULL,0,MODE_HIGH,BarsTSL,BarShift)]+Spread);         
         if(OrderType()==OP_BUY &&
            Bid>OrderOpenPrice() &&
            btsl>OrderStopLoss() &&
            btsl>OrderOpenPrice())   {
