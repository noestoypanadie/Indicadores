//+------------------------------------------------------------------+
//|                                 automated trading program 6c.mq4 |
...
extern string  comment="m ATP.6c";
extern string  sep1="----------------";
...
extern bool    Use.ADR.Tightening (not implemented yet)=false;
extern string  sep2="----------------";
...
CheckForOrderClosing();

TradingEnabled=CheckIfConditionsAllowEntry(); // returns 0 if no trading conditions according to filters and indicators
...
if (Move.To.BreakEven.at.pips!=0) {{MoveToBreakEven();}
Trail.Stop();
comments();
Mail();
return(0);}
...
double Year.Avg.ADR() {
if(DR1==TimeToStr(CurTime(),TIME_DATE)) {return(NormalizeDouble(Year.ADR,Digits));}
Year.ADR=0;
...
{  PosCounter();
   if (b.ticket > 0)
   {  OrderSelect(b.ticket,SELECT_BY_TICKET);
      //Comment(OrderSymbol()," ",OrderStopLoss()," ",OrderOpenPrice()," ",Bid," ",Ask," ",Move.To.BreakEven.Lock.pips*Point);
      if (OrderStopLoss()<OrderOpenPrice())
...
int CheckIfConditionsAllowEntry() // 0=no entry 1=ok for long 2=ok for short
{
   ...
   if (Use.Trading.Hours.Restriction && (Hour()<Start.Trading.Hour.Begin || Hour()>Start.Trading.Hour.End)) return (0); // Check Time Filter
...
{  bool iShouldExit=false;
   ...
   if (b.ticket>0)
   {  if (Exit.On.Yellow && ((t2c==1 || t3c==1 || t4c==1) && (t0p==1 || t1p==1))) {iShouldExit=true;}   
   ...
   if (s.ticket>0)
   {  if (Exit.On.Yellow && ((t0c==1 || t1c==1 || t4c==1) && (t2p==1 || t3p==1))) {iShouldExit=true;} 
