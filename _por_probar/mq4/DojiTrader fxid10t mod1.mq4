...
double CalcSlBuy()   { return (MathMin((Low[Lowest(NULL,0,MODE_LOW,BarsSL,BarsShift)]-Spread),(Bid-(StopLoss*SPoint)))); }
double CalcSlSell()  { return (MathMax((High[Highest(NULL,0,MODE_HIGH,BarsSL,BarsShift)]+Spread),(Ask+(StopLoss*SPoint)))); }
double CalcTpBuy()   { return (MathMin((High[Highest(NULL,0,MODE_HIGH,BarsTP,BarsShift)]),(Ask+(TakeProfit*SPoint)))); }
double CalcTpSell()  { return (MathMax((Low[Lowest(NULL,0,MODE_LOW,BarsTP,BarsShift)]),(Bid-(TakeProfit*SPoint)))); }
...
int start() {
   ...
   for(c = 0; c < OrdersTotal(); c++ ) {
      ...
      btsl=(Low[Lowest(NULL,0,MODE_LOW,BarsTSL,BarsShift)]-Spread);
      stsl=(High[Highest(NULL,0,MODE_HIGH,BarsTSL,BarsShift)]+Spread);
