...
/*   if(!IsTesting() && c<=0) {
   ...
   invalid=iCustom(Symbol(),0,"Support Resistance",15,144,13,1,5,true,Aqua,DeepPink,Red,DarkOrange,DeepSkyBlue,Lime,0,0);
...
double StopLoss() {
   ...
   for(int o=0;o<ObjectsTotal();o++)  {
      if(blue<red && ObjectGetValueByShift(ObjectName(o),0)<(red-order.buffer)) {
...
double TakeProfit()  {
   ...
   for(int p=0;p<ObjectsTotal();p++)  {
      ...
      if(blue>red && ObjectGetValueByShift(ObjectName(p),0)<(red-order.buffer))  {
         tp=ObjectGetValueByShift(ObjectName(p),0)-spread;}}//sell tp
