...
extern double Percent = 0.015;
extern int iTradeOnFriday = 1; // >0 trades on friday
extern int slip = 100;//exits only
extern double Lots = 0.1; // modified by Renato
extern int TakeProfit = 6;
extern int Stoploss = 270;// total loss on all open positions in pips // modified by Renato
...
extern double DivergenceLimit=0.009;

extern int PipStep = 6;//if position goes this amount of pips against you add another.
...
//SetupGlobalVariables(); --!-- I removed it 
...
Comment("MagicNumber="+MagicNumber+", maxDrawDown=",maxDD,"  maxOrdersOpen=",maxOO," Current Divergence = ",diverge,", Bid=",Bid,", Ask=",Ask);
...
void SetupGlobalVariables()
{
  ...
  case 26910: // EURUSD,M30 // optmized 06/03/20  -------------!!!!!!!!!! 26885 
...
//     Stoploss=300;
     ...
     break;
  case 25803: // GBPUSD,M30  --------!!!!!!!!!!!!!!!!! 25778 for My InterbankFX Mini
//     MA_length=18;
//     MA_timeframe=30;
//     MAtype=0;
     Percent=0.25;
...
double iFXAnalyser(int FXA_Period, int mode, int shift)// Made local function by MrPip
{
 ...
 double ind_buffer0, ind_buffer1, ind_buffer2;
   ...
   {
// MODE_DIV
      case 0 : ind_buffer0=iMA(NULL,FXA_Period,FastMA,0,MODE_SMA,Fast_MAMode,shift)
                          -iMA(NULL,FXA_Period,SlowMA,0,MODE_SMA,Slow_MAMode,shift);
                     return (ind_buffer0);
                     break;

//---- Shaun's Slope counted in the 2-nd buffer
// MODE_SLOPE
      case 1 : ind_buffer1=(iMA(NULL,FXA_Period,FastMA,0,MODE_SMA,Fast_MAMode,shift)
                           -iMA(NULL,FXA_Period,SlowMA,0,MODE_SMA,Slow_MAMode,shift))
                           -(iMA(NULL,FXA_Period,FastMA,0,MODE_SMA,Fast_MAMode,shift+1)
                            -iMA(NULL,FXA_Period,SlowMA,0,MODE_SMA,Slow_MAMode,shift+1));
                       return (ind_buffer1);
                       break;
                       
//---- Shaun's Slope of Slope counted in the 3-3d buffer
// MODE_ACEL
      case 2 : ind_buffer2=((iMA(NULL,FXA_Period,FastMA,0,MODE_SMA,Fast_MAMode,shift)
                            -iMA(NULL,FXA_Period,SlowMA,0,MODE_SMA,Slow_MAMode,shift))
                           -(iMA(NULL,FXA_Period,FastMA,0,MODE_SMA,Fast_MAMode,shift+1)
                            -iMA(NULL,FXA_Period,SlowMA,0,MODE_SMA,Slow_MAMode,shift+1))
                           -(iMA(NULL,FXA_Period,FastMA,0,MODE_SMA,Fast_MAMode,shift+1)
                            -iMA(NULL,FXA_Period,SlowMA,0,MODE_SMA,Slow_MAMode,shift+1))
                           -(iMA(NULL,FXA_Period,FastMA,0,MODE_SMA,Fast_MAMode,shift+2)
                            -iMA(NULL,FXA_Period,SlowMA,0,MODE_SMA,Slow_MAMode,shift+2)));
                        return (ind_buffer2);
                        break;
       }
}
