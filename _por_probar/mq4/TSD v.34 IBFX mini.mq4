//+------------------------------------------------------------------+
//|                                   TSD_MR_Trade_OsMA_WPR_0_34.mq4 |
//|                                       Copyright ® 2005 Mindaugas |
//|                                           TSD Trade version 0.34 |
//|                                                                  |
...
extern bool TradeOnInit = true;
// Whether to put new orders or only track old ones. Default is to put new orders.
extern bool NewTrades = true;

// which indicators to use
extern int DirectionMode = DIRECTION_OSMA, FilterMode = FILTER_WPR;
// trading periods
extern int PeriodDirection = PERIOD_W1, PeriodTrade = PERIOD_D1, PeriodTrailing = PERIOD_H4, CandlesTrailing = 3;
// currency pairs to trade
string pairs[] = { "AUDUSDm", "EURCHFm", "EURGBPm", "EURJPYm", "EURUSDm",/* "GBPCHF m", "GBPJPY m",*/ "GBPUSDm",
                   "USDCADm", "USDCHFm", "USDJPYm" };

// MM and broker parameters
extern int Broker = BROKER_ALPARI;
extern double Risk = 0.00, Lots = 0.1;
...
*/
   ...
   if(TradeSymbol=="AUDUSDm")Spread=5 * SPoint;
   if(TradeSymbol=="EURCHFm")Spread=7 * SPoint;
   if(TradeSymbol=="EURGBPm")Spread=4 * SPoint; 
   if(TradeSymbol=="EURJPYm")Spread=4 * SPoint; 
   if(TradeSymbol=="EURUSDm")Spread=2 * SPoint; 
   if(TradeSymbol=="GBPCHFm")Spread=8 * SPoint; 
   if(TradeSymbol=="GBPJPYm")Spread=8 * SPoint; 
   if(TradeSymbol=="GBPUSDm")Spread=4 * SPoint;
   if(TradeSymbol=="USDCADm")Spread=5 * SPoint;
   if(TradeSymbol=="USDCHFm")Spread=4 * SPoint;
   if(TradeSymbol=="USDJPYm")Spread=3 * SPoint;
...
double PipCost (string TradeSymbol) {
   ...
   if      ( TradeSymbol == "EURCHFm" || 
             TradeSymbol == "USDCHFm" )  Cost = 10/  MarketInfo("USDCHF m",MODE_BID);
   else if ( TradeSymbol == "EURGBPm" )  Cost = 10*  MarketInfo("GBPUSD m",MODE_BID);
   else if ( TradeSymbol == "CHFJPYm" ||
             TradeSymbol == "EURJPYm" ||
             TradeSymbol == "USDJPYm" )  Cost = 1000/MarketInfo("USDJPY m",MODE_BID);
   else if ( TradeSymbol == "AUDUSDm" ||
             TradeSymbol == "EURUSDm" ||
             TradeSymbol == "GBPUSDm" ||
             TradeSymbol == "NZDUSDm" )  Cost = 10;
   else if ( TradeSymbol == "GBPCHFm" )  Cost = PipCost("GBPUSDm")     / MarketInfo("USDCHFm",MODE_BID);
   else if ( TradeSymbol == "GBPJPYm" )  Cost = PipCost("GBPUSDm")*100 / MarketInfo("USDJPYm",MODE_BID);
   else if ( TradeSymbol == "EURCADm" ||
             TradeSymbol == "USDCADm" )  Cost = 10/  MarketInfo("USDCAD m",MODE_BID);
   else                                 Cost = 10;
   ...
   if ( Broker == BROKER_ALPARI || Broker == BROKER_METAQUOTES ||
      ...
      if      ( TradeSymbol == "AUDUSDm" )  Cost = 20;
      else if ( TradeSymbol == "GBPUSDm" )  Cost = 7;
   }
