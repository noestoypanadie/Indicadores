...
// stache added tradetime to v03m and calleit v93m_TD
...
extern string UseTradeTime?_is    =            "if True start trades within time specified";
extern bool    UseTradeTime?      =    true;
extern string  ServerStartTime1   = " 06:01"; 	// Start trades after time
extern string  ServerStopTime1    = " 12:59"; 	 // Stop trading after time
extern string  ServerStartTime2   = " 14:01"; 	// Start trades after time
extern string  ServerStopTime2    = " 22:58"; 	 // Stop trading after time
...
bool     YesStop;                        // used to stop trading during certain times
...
bool CheckTradeTime1()
{
  ...
  if(UseTradeTime?)
  {
    if(Month() >=4 && Month()<=10)
      {
       if(TradeTime  < StrToTime(ServerStartTime1)) 
       YesStop=true;
       
       if(TradeTime >= StrToTime(ServerStopTime2))
       YesStop = true;
      }
    if (Month()< 4 || Month() > 10)
      {
       if(TradeTime  < StrToTime(ServerStartTime1)+ 3600) 
       YesStop=true;
       
       if(TradeTime >= StrToTime(ServerStopTime2) + 3600)
       YesStop = true;
      }   
   }  
    return (YesStop);
...
bool CheckTradeTime2()
{
  ...
  if(UseTradeTime?)
  {
    if(Month() >=4 && Month()<=10)
      {
      if(TradeTime >= StrToTime(ServerStopTime1) && 
       TradeTime < StrToTime(ServerStartTime2))
       YesStop = true;
      }
   if (Month()< 4 || Month() > 10)  
      {
      if(TradeTime >= StrToTime(ServerStopTime1)+3600 && 
       TradeTime < StrToTime(ServerStartTime2)+3600)
       YesStop = true;
      }  
   } 
...
int start()
  {
   ...
   if(Close[0]>bux && volumeOK && TradeDay  &&!CheckTradeTime1() && !CheckTradeTime2() ) 
     {
      SELLme=true; 
   ...
   if(Close[0]<bdx && volumeOK && TradeDay && !CheckTradeTime1() && !CheckTradeTime2()) 
     {
      BUYme=true;  
   ...
   //+------------+
    ...
    if (CheckTradeTime1()) 
      ...
      {  
        Comment ( TradeComment + " on ", Symbol()+" "+
             " is on Wine Break.... Hope to stumble back to trading on ServerTime "+ 
              ServerStartTime1 );
         return(0);     
      }
    ...
    if (CheckTradeTime2()) 
      {
        Comment ( TradeComment + " on ", Symbol()+" "+
             " is out to Lunch.... Hope to return to trading on ServerTime "+ 
              ServerStartTime2 );
         return(0);     
       } 
   ...
   //ENTRY LONG (buy, Ask) 
   if( TradeAllowed && BUYme && !CheckTradeTime1() && !CheckTradeTime2())
     {
      while(true)
        {
   ...
   if( TradeAllowed && SELLme && !CheckTradeTime1() && !CheckTradeTime2())
     {
      while(true)
        {
         if( AccountFreeMargin()< (AccountBalance()*(MinFreeMarginPct/100)) )
