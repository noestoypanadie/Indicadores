...
Lotsf = MathRound(AccountBalance()/40000);
...
if (newbar != Time[0]) 
{
	   ...
	   if(Direction == 1 && WilliamsBuy)
		{
			...
			{
				BuyEntryOrderTicket=OrderSend(Symbol(),OP_BUYSTOP,Lotsf,PriceOpen,Slippage,Low[1] - 1 * Point,PriceOpen + TakeProfit * Point,"Buy Entry Order placed at "+CurTime(),0,0,Green);
				return(0);
     ...
     if(Direction == -1 && WilliamsSell)
     {
         PriceOpen = Low[1] - 1 * Point;
			...
			{
				SellEntryOrderTicket=OrderSend(Symbol(),OP_SELLSTOP,Lotsf,PriceOpen,Slippage,High[1] + 1 * Point,PriceOpen - TakeProfit * Point,"Sell Entry Order placed at "+CurTime(),0,0,Green);
				return(0);
			} // close for if(PriceOpen < (Bid - 16 * Point))
			else
			{
				NewPrice = Bid - 16 * Point;
				SellEntryOrderTicket=OrderSend(Symbol(),OP_SELLSTOP,Lotsf,NewPrice,Slippage,High[1] + 1 * Point,NewPrice - TakeProfit * Point,"Sell Entry Order placed at "+CurTime(),0,0,Green);
            return(0);			
			} // close for else statement
