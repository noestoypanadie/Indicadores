...
extern string nameEA         = "FarhadCrab-H1.mq4";// To "easy read" which EA place an specific order and remember me forever :)
...
double faMA3               = iMA(NULL,0,9,0,MODE_SMA,PRICE_TYPICAL,0);
double stochHistCurrent    = iStochastic(NULL,PERIOD_H4,5,3,3,MODE_SMA,0,MODE_MAIN,0);
...
//Check Margin Requirement
   ...
   if (!(stochHistCurrent<20)){
      if ((faClose2<faMA3)){
         OpenSell();}}
   if (!(stochHistCurrent>80)){
      if ((faClose0>faMA1)){
         OpenBuy();}}
...
double GetTakeProfitSell() { return(Bid-sTakeProfit*Point); } 
  void calculateIndicators() {  
   ...
   stochHistCurrent    = iStochastic(NULL,PERIOD_H4,5,3,3,MODE_SMA,0,MODE_MAIN,0);
   ...
   closePreviousD      = iClose(NULL,PERIOD_H1,2); //Close Price Previous for D1 TimeFrame
   maLongCurrent       = iMA(NULL,PERIOD_H1,55,1,MODE_SMMA,PRICE_TYPICAL,0); //Current Long Term Moving Average 
   maLongPrevious      = iMA(NULL,PERIOD_H1,55,1,MODE_SMMA,PRICE_TYPICAL,2); //Previous Long Term Moving Average 
