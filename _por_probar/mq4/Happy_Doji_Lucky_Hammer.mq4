...
string            comment;          comment     ="HDLH v.019";
   ...
   if(TotalTradesThisSymbol(TradeSymbol)>0)  {
      for(cnt=0;cnt<total;cnt++) {
         ...
         if(OrderSymbol()==TradeSymbol)   {
            ...
            OrderSelect(dcc,SELECT_BY_TICKET);
            if(OrderCloseTime()>0) {dcc=0;}
            OrderSelect(ppt,SELECT_BY_TICKET);
            if(OrderCloseTime()>0) {ppt=0;}
         }//end if(OrderSymbol
      }//end for
...
bool darkcloudcover()   {  if(Open[1]>=(Open[Highest(NULL,0,MODE_OPEN,BarsSL,0)]) &&
                              Close[1]>Open[2] &&
                              Open[2]<Close[2] &&
                              Open[1]>Close[1] &&
...
bool piercingpattern()  {  if(Open[1]<=(Open[Lowest(NULL,0,MODE_OPEN,BarsSL,0)]) &&
                              Close[1]<Open[2] &&
                              Open[2]>Close[2] &&
                              Open[1]<Close[1] &&
...
int TotalTradesThisSymbol(string TradeSymbol) {
   ...
   for(i=0;i<OrdersTotal();i++)  {
      ...
      if(OrderSymbol()==TradeSymbol &&
         ...
         OrderMagicNumber()==888 ||
         OrderMagicNumber()==999 ||
         OrderMagicNumber()==1111)   {  TradesThisSymbol++;  }
   }//end for
