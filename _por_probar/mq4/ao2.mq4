...
if(Period()!=15 && c<=0) {
int ret=MessageBox("The AO expert was designed for 15m\n charts,do you wish to continue?","ao expert advisor",MB_YESNO|MB_ICONQUESTION);
c++;
if(ret==IDNO) return(0);}}

double mafc=iMA(Symbol(),0,5,0,MODE_SMA,PRICE_MEDIAN,0);
double masc=iMA(Symbol(),0,34,0,MODE_SMA,PRICE_MEDIAN,0);
double mafp=iMA(Symbol(),0,5,0,MODE_SMA,PRICE_MEDIAN,1);
double masp=iMA(Symbol(),0,34,0,MODE_SMA,PRICE_MEDIAN,1);


PosCounter();
double aoc=mafc-masc;
double aop=mafp-masp;
//double AO=iCustom(Symbol(),0,"Awesome",0,0,0);

if((aop<aoc)/* && (aop<(0.0000))*/ && b==0) {
...
if(!IsTesting()) {Comment("AO Last:",AOC()," ",aoc,"\n"," AO Prev:",AOP()," ",aop);}
...
double AOC()   {
return(iMA(Symbol(),0,5,0,MODE_SMA,PRICE_MEDIAN,0)-iMA(Symbol(),0,34,0,MODE_SMA,PRICE_MEDIAN,0));}

double AOP()   {
return(iMA(Symbol(),0,5,0,MODE_SMA,PRICE_MEDIAN,1)-iMA(Symbol(),0,34,0,MODE_SMA,PRICE_MEDIAN,1));}
