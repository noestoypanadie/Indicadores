...
string Name_Expert = "Firebird v63H"; // included by Renato
string NameFileSound = "expert.wav"; // included by Renato
extern int MAXTrades=3; // maximum number of trades to have open at once
...
//Ron Adds
int Diverge=divergence(Fast_Period,Slow_Period,Fast_Price,Slow_Price,0)/Point;
...
int Slope=Diverge-divergence(Fast_Period,Slow_Period,Fast_Price,Slow_Price,1)/Point;

Comment("maxDD=",maxDD," - maxOO=",maxOO," - CurDiv=",Slope);
...
//if ( MathAbs(diverge)>DivergenceLimit ) Comment("Divergence!"); else {

if ( Hour()<OpeningHour+GMT+DST || Hour()>ClosingHour+GMT+DST ) Comment("bad hours.");  else { // included by Renato
if ( AccountFreeMargin()<100*Lots ) Comment("Not enough margin!"); else {
...
//   if((myMA*(1+Percent/100))<Bid) Print(" Top, Bid ",myMA*(1+Percent/100),"  ",Bid);
   ...
   if ( (myMA*(1+Percent/100))<Bid && Direction!=22 && (Bid>=(LastPrice+(CurrentPipStep*Point)) || MyOrdersTotal()==0) && Diverge<-DivergenceLimit && Slope>0 && RVI<0 ) // modified by Renato
 	  {
   ...
   if ( (myMA*(1-Percent/100))>Ask && Direction!=11 && (Ask<=(LastPrice-(CurrentPipStep*Point)) || MyOrdersTotal()==0) && Diverge>DivergenceLimit && Slope<0 && RVI>0 ) // Go LONG -> Only buy if >= 30 pips below previous position entry // modified by Renato
     {
...
} // end of margin test // included by Renato
