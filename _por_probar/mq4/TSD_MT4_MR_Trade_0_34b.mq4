//+------------------------------------------------------------------+
//|                                  TSD_MR_Trade_OsMA_WPR_0_34b.mq4 |
//|                                       Copyright ® 2005 Mindaugas |
//|                                          TSD Trade version 0.34b |
//|                                                                  |
...
//| o Variable size buffer by Jacob Yego, implemented by Matt Kennel |
//+------------------------------------------------------------------+
...
#define  BUFFER_OBRIEN 1   // standard 1 point buffer
#define  BUFFER_YEGO   2   // yego's modification 
#define  BUFFER_CAMARILLA 3 // enter at "camarilla" breakout points? 
...
// Offset in opening trades
int BufferMode = BUFFER_OBRIEN; // OBRIEN;
...
int Broker = BROKER_IBFX;
double Risk = 0.00, Lots = 1.0; // 0.1;

// parameters for MACD and OsMA
int DirectionFastEMA = 12, DirectionSlowEMA = 26, DirectionSignal = 9;
//int DirectionFastEMA = 3, DirectionSlowEMA = 13, DirectionSignal = 9;
...
int init() {
   ...
   if ( IsTesting() ) { if ( ArrayResize(pairs,1) != 0 )  pairs[0] = Symbol(); Print("DEBUG: IsTesting==true"); }
   else {
...
int start() {
   ...
   // Print("DEBUG: Pair = " +Pair+" TradeSymbol = "+TradeSymbol); 
   
   if ( SymbolExists(TradeSymbol)==false ) {
      if ( ! Alerted )  Alert ("No data for the pair: ", TradeSymbol, ". Please review and fix pairs variable.");
      Alerted = true;
      Print("DEBUG: symbol does not exist for pair: "+TradeSymbol); 
      return(0);
   }
   ...
   if (MarketInfo(TradeSymbol,MODE_SPREAD) > 0) {
   ...
   // Print("DEBUG: Direction = " + Direction + " ValueComment = " +ValueComment); 
   
   CommentAll (Direction, ValueComment);

   if ( ! IsTradeAllowed() )  {
      Print("DEBUG: IsTradeAllowed == false");
      return(0);
   }
   ...
   ///////////////////////////////////////////////////////////////////////////////
   //Print("DEBUG: calling NewBar()"); 
   if ( NewBar() ) {
      // Print("DEBUG: in NewBar()==true"); 
      /////////////////////////////////////////////////
      //  Place new order
      /////////////////////////////////////////////////
      if ( TradesThisSymbol < 1 && NewTrades ) {
         // Print("DEBUG: in place new order."); 
         ticket = 0;
...
//+------------------------------------------------------------------+
double CalcOpenBuy  () { 
   double H = iHigh(TradeSymbol, PeriodTrade, 1);
   double L = iLow(TradeSymbol, PeriodTrade, 1);
   double Range = H-L; 
   double p;
   
   if (BufferMode == BUFFER_OBRIEN) {
      p = H+1*SPoint;
   } else if (BufferMode == BUFFER_YEGO) {
      double YegoBuffer = MathAbs(Range - iATR(TradeSymbol,PeriodTrade,2,1));
      p = H+MathMax(YegoBuffer,1*SPoint); 
   } else if (BufferMode == BUFFER_CAMARILLA) {
      p = Range*1.1/2 + iClose(TradeSymbol,PeriodTrade,1);
   }
   return (MathMax (NormalizeDouble(p,SDigits) + Spread, MarketInfo(TradeSymbol, MODE_ASK) + StopLevel)); 
}
...
double CalcOpenSell () {
   ...
   if (BufferMode == BUFFER_OBRIEN) {
      ...
      double YegoBuffer = MathAbs(Range - iATR(TradeSymbol,PeriodTrade,2,1));
      p = L-MathMax(YegoBuffer,1*SPoint); 
   } else if (BufferMode == BUFFER_CAMARILLA) {
      p = -Range*1.1/2 + iClose(TradeSymbol,PeriodTrade,1);
   }
   return (MathMin (NormalizeDouble(p,SDigits), MarketInfo(TradeSymbol, MODE_BID) - StopLevel)); 
} 
...
//+------------------------------------------------------------------+
bool SymbolExists(string TradeSymbol) {
   if (Broker == BROKER_IBFX) {
      // IBFX demo appears to always return 0 for MODE_SPREAD, even if
      // pairs are OK. 
      return(MarketInfo(TradeSymbol,MODE_DIGITS) != 0);
   } else {
      return(MarketInfo (TradeSymbol, MODE_SPREAD) >= 1);
   }
}
...
bool NewBar () {
   ...
   //Print("DEBUG: PairNewBar[Pair]="+PairNewBar[Pair]); 
   if ( PairNewBar[Pair] < 3 ) {
      PairNewBar[Pair]++;
      // Print("DEBUG: NewBar == true"); 
      return(true);
   }
   
   //Print("DEBUG: PairTime[Pair] = "+PairTime[Pair]+" iTime() ="+iTime (TradeSymbol, PeriodTrade, 0 )); 
  
   if ( PairTime[Pair] != iTime (TradeSymbol, PeriodTrade, 0 ) ) {
      PairTime[Pair] = iTime (TradeSymbol, PeriodTrade, 0 );
      PairNewBar[Pair] = 0;
      // Print("DEBUG: Setting PairNewBar[] to zero for Pair="+Pair); 
   }
   // Print("DEBUG: NewBar == false"); 
   return(false);
}
//+------------------------------------------------------------------+
