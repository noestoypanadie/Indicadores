...
int start() {
   ...
   for(cnt=0;cnt<total;cnt++) {
      ...
      if(OrderType()==OP_BUY && IsTradeAllowed() &&
         OrderSymbol()==Symbol() && OrderMagicNumber()==21 ||
         OrderMagicNumber()==41 || OrderMagicNumber()==61) {
      ...
      if(OrderType()==OP_SELL && IsTradeAllowed() &&
         OrderSymbol()==Symbol() && OrderMagicNumber()==11 ||
         OrderMagicNumber()==31 || OrderMagicNumber()==51)   {
         if(MaElineTSL==0) {TSL=NormalizeDouble(ma,Digits); }
