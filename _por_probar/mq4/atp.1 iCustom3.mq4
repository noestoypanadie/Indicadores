...
extern double  tsl.divisor=0.618;
extern int     stoploss.pips=100;
extern bool    Use.ADR.for.SL.pips?=true;

extern bool    Use.Money.Management?=true;//if false, lot=Minimum.Lot
extern double  Minimum.Lot=0.01;
extern double  MaximumRisk=0.015;
...
if(/*OrderProfit()>0 &&*/ (f12c==1 || f13c==1) && (f10p==1 || f11p==1)) {OrderClose(OrderTicket(),OrderLots(),Bid,slip,Aqua);}
OrderSelect(s.ticket,SELECT_BY_TICKET);
if(/*OrderProfit()>0 &&*/ (f10c==1 || f11c==1) && (f12p==1 || f13p==1)) {OrderClose(OrderTicket(),OrderLots(),Ask,slip,Magenta);}
Mail();
Trail.Stop();
...
if(OrderCloseTime()>0)  {
   ...
   DoubleToStr(AccountBalance(),2)+" $"+DoubleToStr(AccountFreeMargin(),2));}
OrderSelect(s.ticket,SELECT_BY_TICKET);
if(OrderCloseTime()>0)  {
