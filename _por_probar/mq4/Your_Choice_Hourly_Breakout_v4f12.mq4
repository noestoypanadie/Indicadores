...
extern int    ShowComments         =    0; // 1 = Yes  ,  0 = No .

int    Magic_Num            = 13;

//- Do not change these values , scroll down to  if ( Symbol() == "++++++" ) .
...
int    AtrTrailing,
       AtrPeriods;
double TrailAt_TimesATR;
int    StandardTrailAmt;
...
buystop  = (((MathRound((High[Highest(Symbol(),0,MODE_HIGH,PeriodsLookback,1)])/Point))*Point)+(rdhidiff))+(spread*2);
...
if (ShowComments != 0 &&
 ...
 { Comment("\n","  Chart Hour:  =  ",Hour()," ,  FrstSessHour:  =  ",FrstSessHour," ,  SecondSessHour:  =  ",SecondSessHour,
           ...
           "\n",
           "\n","  BuyStop ( Highest High of PeriodsLookback + rdhidiff + Spread*2) :  =  ",buystop,
...
if ( Hour() == FrstSessHour || Hour() == SecondSessHour )
 {
  if ( buyorder == 0 && byok <= 0 && deleteall < 0 )
   {
     ticket = OrderSend(Symbol(),OP_BUYSTOP,Lots,buystop,0,buystop-MaxLoss*Point,buystop+MaxProfit*Point,"Your_Choice_Hourly_Breakout_v4f",Magic_Num,0,Lime);
  ...
  if ( sellorder == 0 && slok <= 0 && deleteall < 0 )
   {
     ticket = OrderSend(Symbol(),OP_SELLSTOP,Lots,sellstop,0,sellstop+MaxLoss*Point,sellstop-MaxProfit*Point,"Your_Choice_Hourly_Breakout_v4f",Magic_Num,0,Red);
      ...
      if ( OrderStopLoss() < OrderOpenPrice()-1*Point && OrderClosePrice()-OrderOpenPrice() > Adj2B.E.Aftr_Pips*Point )
       {
         ...
         Print("Adjusted To Breakeven Plus  ",Symbol());
         return(0);
       }
     }
    if (OrderSymbol()==Symbol() && OrderMagicNumber()==Magic_Num && OrderType()==OP_SELL)
      ...
      if (OrderClosePrice()-OrderOpenPrice() > pips)
       {
         ...
         Print("Order Closed At ATR TakeProfit  ",Symbol());
         return(0);
       }
     }
    if (OrderSymbol()==Symbol() && OrderMagicNumber()==Magic_Num && OrderType()==OP_SELL)
