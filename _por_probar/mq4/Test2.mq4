      ...
      if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) { 
         if (OrderSymbol()==Symbol()) { 
            if (OrderType()==OP_BUY) { 
                     ...
                     if (IsTesting() && LastLogTime != iTime(NULL,0,1)) 
                     {
                        LastLogTime=iTime(NULL,0,1);
                        if (OrderTakeProfit()==OrderOpenPrice()+lTakeProfit*Point)
                           OrderModify(OrderTicket(),OrderOpenPrice(),OrderStopLoss(),OrderTakeProfit()+Point*1,0,Cyan);
                     }     else OrderModify(OrderTicket(),OrderOpenPrice(),OrderStopLoss(),OrderOpenPrice()+lTakeProfit*Point,0,Cyan);
      ...
      if (OrderSelect(i, SELECT_BY_POS, MODE_TRADES)) { 
         if (OrderSymbol()==Symbol()) { 
            if (OrderType()==OP_SELL) { 
                     ...
                     {
                        LastLogTime=iTime(NULL,0,1);
                        if (OrderTakeProfit()==OrderOpenPrice()-sTakeProfit*Point)
                           OrderModify(OrderTicket(),OrderOpenPrice(),OrderStopLoss(),OrderTakeProfit()+Point*1,0,Cyan);
                     }     else OrderModify(OrderTicket(),OrderOpenPrice(),OrderStopLoss(),OrderOpenPrice()-sTakeProfit*Point,0,Cyan);
