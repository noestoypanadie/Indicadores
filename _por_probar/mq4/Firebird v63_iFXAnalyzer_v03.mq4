...
extern double Percent = 0.148;
extern int TradeOnFriday =1; // >0 trades on friday
extern int slip = 100;//exits only
extern double Lots = 0.3;
extern int TakeProfit = 73;
extern int Stoploss = 90;// total loss on all open positions in pips

// Ron added for iFXAnalyzer
extern int    Fast_Period=10;
extern int    Fast_Price = PRICE_OPEN;
extern int    Slow_Period=100;
extern int    Slow_Price = PRICE_OPEN;
extern double DivergenceLimit=0.0033;
double diverge;
//      if(pos==0)
//        {
//         tickcount++;
//         iFileHandle = FileOpen("iDivergence", FILE_CSV|FILE_WRITE, ",");
//         FileSeek(iFileHandle, 0, SEEK_END);
//         FileWrite(iFileHandle, tickcount," ",maF1-maS1);
...
extern int PipStep = 9;  //if position goes this amount of pips against you add another.
...
if(OrdersTotal()>maxOO) maxOO=OrdersTotal();
...
//   if((myMA*(1+Percent/100))<Bid) Print(" Top, Bid ",myMA*(1+Percent/100),"  ",Bid);
   ...
   if((myMA*(1+Percent/100))<Bid && Direction!=22 && (Bid>=(LastPrice+(CurrentPipStep*Point))||total==0)) 
 	  {
 	   // Ron added divergence check
 	   if(diverge>=(DivergenceLimit*(-1)) && diverge<=DivergenceLimit)
 	     {
   ...
   if((myMA*(1-Percent/100))>Ask && Direction!=11 && (Ask<=(LastPrice-(CurrentPipStep*Point))||total==0)) 	 
     {
 	   // Ron added divergence check
 	   if(diverge>=(DivergenceLimit*(-1)) && diverge<=DivergenceLimit)
 	     {
