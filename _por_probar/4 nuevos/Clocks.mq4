...
extern  int      ClockCorner       = 1;
extern  int      ClockXOffset      = 20;
extern  int      ClockYOffset      = 250;
extern  int      RefreshEveryXMins = 0;
extern  color    NightColor        = DarkSlateGray;
extern  color    DayColor          = Gray;
//extern  color    MktOpenColor      = White;
extern  string   LabelFont         = "Arial Black";
extern  int      LabelSize         = 11;
extern  string   MT4LabelFont      = "Tahoma";
extern  int      MT4LabelSize      = 10;
extern  int      ClockSize         = 60;

extern  int      Clock1_TimeDiff   = 17;
extern  string   Clock1_Label      = "USD"; 
extern  color    Clock1_Color      = White;

extern  int      Clock2_TimeDiff   = 17; 
extern  string   Clock2_Label      = "CAD"; 
extern  color    Clock2_Color      = SaddleBrown;

extern  int      Clock3_TimeDiff   = 22; 
extern  string   Clock3_Label      = "GBP"; 
extern  color    Clock3_Color      = Crimson;

extern  int      Clock4_TimeDiff   = 23; 
extern  string   Clock4_Label      = "EUR"; 
extern  color    Clock4_Color      = HotPink;

extern  int      Clock5_TimeDiff   = 23; 
extern  string   Clock5_Label      = "CHF"; 
extern  color    Clock5_Color      = Yellow;

extern  int      Clock6_TimeDiff   = 6;
extern  string   Clock6_Label      = "CNY"; 
extern  color    Clock6_Color      = Gray;

extern  int      Clock7_TimeDiff   = 7; 
extern  string   Clock7_Label      = "JPY"; 
extern  color    Clock7_Color      = DarkViolet;

extern  int      Clock8_TimeDiff   = 9; 
extern  string   Clock8_Label      = "AUD"; 
extern  color    Clock8_Color      = Green;

extern  int      Clock9_TimeDiff   = 11; 
extern  string   Clock9_Label      = "NZD"; 
extern  color    Clock9_Color      = CornflowerBlue;

string   ccy, IndiName;
int      tmf, dig, bStyle, bWidth;
double   pnt, askp, bidp, spr, tickval;
datetime prev_time;
...
//+------------------------------------------------------------------+
  ...
  IndiName = "Clocks-" + NumberToStr(checksum,"Z7");
...
int plot_obj()  {
//+------------------------------------------------------------------+
  int      ClockX[10]     = {0,80,160,240,320,0,80,160,240,320};
  int      ClockY[10]     = {0,0,0,0,0,100,100,100,100,100};
  int      ClockTime[10]  = {0,17,17,22,23,11,23,7,9,11};
  string   ClockName[10]  = {"MT4","USD","CAD","GBP","EUR","LCL","CHF","JPY","AUD","NZD"};
  color    ClockColor[10] = {DimGray,White,SaddleBrown,Crimson,HotPink,Silver,Yellow,Purple,Green,CornflowerBlue};

  ClockTime[1] = Clock1_TimeDiff;     ClockName[1] = Clock1_Label;     ClockColor[1] = Clock1_Color;
  ClockTime[2] = Clock2_TimeDiff;     ClockName[2] = Clock2_Label;     ClockColor[2] = Clock2_Color;
  ClockTime[3] = Clock3_TimeDiff;     ClockName[3] = Clock3_Label;     ClockColor[3] = Clock3_Color;
  ClockTime[4] = Clock4_TimeDiff;     ClockName[4] = Clock4_Label;     ClockColor[4] = Clock4_Color;
  ClockTime[5] = Clock5_TimeDiff;     ClockName[5] = Clock5_Label;     ClockColor[5] = Clock5_Color;
  ClockTime[6] = Clock6_TimeDiff;     ClockName[6] = Clock6_Label;     ClockColor[6] = Clock6_Color;
  ClockTime[7] = Clock7_TimeDiff;     ClockName[7] = Clock7_Label;     ClockColor[7] = Clock7_Color;
  ClockTime[8] = Clock8_TimeDiff;     ClockName[8] = Clock8_Label;     ClockColor[8] = Clock8_Color;
  ClockTime[9] = Clock9_TimeDiff;     ClockName[9] = Clock9_Label;     ClockColor[9] = Clock9_Color;

  for (int i=0; i<10; i++) {
    int hr = MathMod(Hour()+ClockTime[i],24);
    if (hr>5 && hr<18) bool dn=true; else dn=false;
    if (hr>7 && hr<17) bool mo=true; else mo=false;
    hr = MathMod(hr,12);
    if (hr==0) hr=12; 
    ...
    ObjectSet(objname,OBJPROP_XDISTANCE,ClockXOffset+ClockX[i]*ClockSize/60);
    ObjectSet(objname,OBJPROP_YDISTANCE,ClockYOffset+ClockY[i]*ClockSize/60);
    ObjectSet(objname,OBJPROP_CORNER,ClockCorner);
    if (mo && i>0)
      ObjectSetText(objname,StringSetChar(" ",0,182+hr),ClockSize,"Wingdings",ClockColor[i]);
    else if (dn)
      ObjectSetText(objname,StringSetChar(" ",0,182+hr),ClockSize,"Wingdings",DayColor);
    else
      ObjectSetText(objname,StringSetChar(" ",0,182+hr),ClockSize,"Wingdings",NightColor);

    objname = IndiName + "-t-" + i;
    string objtext  = ClockName[i];
    string objfont  = LabelFont;
    int    objfsize = LabelSize;
    int    xpos     = ClockXOffset+(ClockX[i]+20)*ClockSize/60;
    if (objtext == "MT4")  {
      objtext  = DateToStr(TimeCurrent(),":I:S");
      objfont  = MT4LabelFont;
      objfsize = MT4LabelSize;
      xpos     = ClockXOffset+(ClockX[i]+14)*ClockSize/60;
    }  
    ObjectDelete(objname);
    ObjectCreate(objname,OBJ_LABEL,0,0,0);
    ObjectSet(objname,OBJPROP_XDISTANCE,xpos);
    ObjectSet(objname,OBJPROP_YDISTANCE,ClockYOffset+(ClockY[i]+77)*ClockSize/60);
    ObjectSet(objname,OBJPROP_CORNER,ClockCorner);
    if (mo && i>0)
      ObjectSetText(objname,objtext,objfsize,objfont,ClockColor[i]);
    else if (dn || i==0)
      ObjectSetText(objname,objtext,objfsize,objfont,DayColor);
    else  
      ObjectSetText(objname,objtext,objfsize,objfont,NightColor);
 }   
  return(0);
}
