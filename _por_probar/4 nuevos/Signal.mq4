...
extern int     scaleX               = 16,          // horizontal interval at which the squares are created
               scaleY               = 10,          // vertical interval
               offsetX              = 35,          // horizontal indent of all squares
               offsetY              = 30,          // vertical indent
               fontSize             = 10,          // font size
               corner               = 1,           // adding a parameter for choosing a corner
...
extern color   signalBuyColor       = Green,       // color of the symbol of a buy signal
               ...
               textColor            = White;       // color of all writings
extern int     MA1period            = 13;               
extern int     MA2period            = 24;               
extern int     WPRperiod            = 14;               
extern int     RefreshEveryXMins    = 0;               
                
int      period[]           = {1,5,15,30,60,240,1440,10080,43200};
string   periodString[]     = {"1","5","15","30","H","4","D","W","M"};
string   signalNameString[] = {"MA","WPR","SAR"};
string   IndiName           = "#signal";
string   ccy;
int      tmf, dig, bStyle, bWidth;
double   pnt, askp, bidp, lastbid, spr, tickval, lswap, sswap;
datetime prev_time;
...
//+------------------------------------------------------------------+
  ...
  for (int x=0; x<9; x++)   {
    for (int y=0; y<3; y++)   {
      string objname = IndiName + NumberToStr(x,"'-XY-'Z2") + NumberToStr(y,"'-'Z2");
      ObjectCreate(objname,OBJ_LABEL,0,0,0,0,0);
      ObjectSet(objname,OBJPROP_CORNER,corner);    
      ObjectSet(objname,OBJPROP_XDISTANCE,x*scaleX+offsetX);
      ObjectSet(objname,OBJPROP_YDISTANCE,y*scaleY+offsetY);
      ObjectSetText(objname,CharToStr(symbolCodeNoSignal),fontSize,"Wingdings",noSignalColor);  
  } }
  for (x=0; x<9; x++)  {
    objname = IndiName + NumberToStr(x,"'-X-'Z2");
    ObjectCreate(objname,OBJ_LABEL,0,0,0,0,0);
    ObjectSet(objname,OBJPROP_CORNER,corner);
    ObjectSet(objname,OBJPROP_XDISTANCE,x*scaleX+offsetX);
    ObjectSet(objname,OBJPROP_YDISTANCE,offsetY-10);
    ObjectSetText(objname,periodString[x],8,"Tahoma",textColor);  
  }  
  for(y=0; y<3; y++)   {
   objname = IndiName + NumberToStr(y,"'-Y-'Z2");
   ObjectCreate(objname,OBJ_LABEL,0,0,0,0,0);
   ObjectSet(objname,OBJPROP_CORNER,corner);
   ObjectSet(objname,OBJPROP_XDISTANCE,offsetX-25);
   ObjectSet(objname,OBJPROP_YDISTANCE,y*scaleY+offsetY);
   ObjectSetText(objname,signalNameString[y],8,"Tahoma",textColor);
  }

  objname = IndiName + "-A";
  ObjectCreate(objname,OBJ_LABEL,0,0,0,0,0);
  ObjectSet(objname,OBJPROP_CORNER,corner);
  ObjectSet(objname,OBJPROP_XDISTANCE,offsetX-20);
  ObjectSet(objname,OBJPROP_YDISTANCE,offsetY+35);

  objname = IndiName + "-B";
  ObjectCreate(objname,OBJ_LABEL,0,0,0,0,0);
  ObjectSet(objname,OBJPROP_CORNER,corner);
  ObjectSet(objname,OBJPROP_XDISTANCE,offsetX-20);
  ObjectSet(objname,OBJPROP_YDISTANCE,offsetY+60+scaleY);
  
  objname = IndiName + "-C";
  ObjectCreate(objname,OBJ_LABEL,0,0,0,0,0);
  ObjectSet(objname,OBJPROP_CORNER,corner);
  ObjectSet(objname,OBJPROP_XDISTANCE,offsetX-20);
  ObjectSet(objname,OBJPROP_YDISTANCE,offsetY+60+3*scaleY);
  
  objname = IndiName + "-D";
  ObjectCreate(objname,OBJ_LABEL,0,0,0,0,0);
  ObjectSet(objname,OBJPROP_CORNER,corner);
  ObjectSet(objname,OBJPROP_XDISTANCE,offsetX-20);
  ObjectSet(objname,OBJPROP_YDISTANCE,offsetY+60+5*scaleY);

  objname = IndiName + "-E";
  ObjectCreate(objname,OBJ_LABEL,0,0,0,0,0);
  ObjectSet(objname,OBJPROP_CORNER,corner);
  ObjectSet(objname,OBJPROP_XDISTANCE,offsetX-20);
  ObjectSet(objname,OBJPROP_YDISTANCE,offsetY+60+7*scaleY);
...
//+------------------------------------------------------------------+
  ...
  for (int x=0; x<9; x++)   {
    int y=0;
    string objname = IndiName + NumberToStr(x,"'-XY-'Z2") + NumberToStr(y,"'-'Z2");
    if (iMA(Symbol(),period[x],MA1period,0,0,0,0) > iMA(Symbol(),period[x],MA2period,0,0,0,0))
      ObjectSetText(objname,CharToStr(symbolCodeBuy),fontSize,"Wingdings",signalBuyColor);
    else
      ObjectSetText(objname,CharToStr(symbolCodeSell),fontSize,"Wingdings",signalSellColor);  
  }
  for (x=0; x<9; x++)   {
    y=1;
    objname = IndiName + NumberToStr(x,"'-XY-'Z2") + NumberToStr(y,"'-'Z2");
    if (MathAbs(iWPR(Symbol(),period[x],WPRperiod,0)) < 20.0)
      ObjectSetText(objname,CharToStr(symbolCodeBuy),fontSize,"Wingdings",signalBuyColor);
    else if(MathAbs(iWPR(Symbol(),period[x],WPRperiod,0)) > 80.0)
      ObjectSetText(objname,CharToStr(symbolCodeSell),fontSize,"Wingdings",signalSellColor);
    else
      ObjectSetText(objname,CharToStr(110),fontSize,"Wingdings",noSignalColor);   
  }
  ...
  double fropen = (iClose(ccy,PERIOD_D1,0) - iOpen(ccy,PERIOD_D1,0))/pnt;
  ...
  objname = IndiName + "-A";
  if (dig>3)
    string objtext = NumberToStr(bidp,"R2.5'   '");
  else
    objtext = NumberToStr(bidp,"R4.3'   '");
  objtext = StringSubstr(objtext,0,7); // + "(" + StringSubstr(objtext,7,1) + ")";
  color colr = textColor;
  if (bidp > lastbid)   colr = Green;
  else
  if (bidp < lastbid)   colr = Red;
  ObjectSetText(objname,objtext,18,"Arial Black",colr);  
  lastbid = bidp;

  objname = IndiName + "-B";
  if (corner==0 || corner==2)
    objtext = NumberToStr(spr,"'S  'TR4.1") + NumberToStr(pctspr,"'  ('TR3.1'%)'");
  else
    objtext = NumberToStr(spr,"TR4.1") + NumberToStr(pctspr,"'  ('TR3.1'%)  S'");
  ObjectSetText(objname,objtext,11,"Verdana",textColor);  

  objname = IndiName + "-C";
  if (corner==0 || corner==2)
    objtext = NumberToStr(tickval,"'$  'TR4.2") + NumberToStr(fropen,"'       'TR+3");
  else  
    objtext = NumberToStr(fropen,"TR+3'       '") + NumberToStr(tickval,"TR4.2'  $'");
  ObjectSetText(objname,objtext,11,"Verdana",textColor);  

  objname = IndiName + "-D";
  if (corner==0 || corner==2)
    objtext = NumberToStr(tdy,"'R  'TR4") + NumberToStr(adr,"' / 'TR4") + NumberToStr(pctadr,"'  ('TR3'%)'");
  else  
    objtext = NumberToStr(tdy,"TR4") + NumberToStr(adr,"' / 'TR4") + NumberToStr(pctadr,"'  ('TR3'%)  R'");
  ObjectSetText(objname,objtext,11,"Verdana",textColor);  

  objname = IndiName + "-E";
  if (corner==0 || corner==2)
    objtext = NumberToStr(lswap,"'C  'TR+2.2") + NumberToStr(sswap,"'   'TR+2.2");
  else  
    objtext = NumberToStr(lswap,"TR+2.2") + NumberToStr(sswap,"'   'TR+2.2'  C'");
  ObjectSetText(objname,objtext,11,"Verdana",textColor);  
